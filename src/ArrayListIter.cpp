...
{
    ArrayListIterator<T> iter(mPtr + offset);
    return iter;
}
...
{
    ArrayListIterator<T> newIter(iter.mPtr + offset);
    return newIter;
}
...
{
    ArrayListConstIterator<T> temp(*this);
    ++*this;
    return temp;
}
...
template <typename T> ArrayListConstIterator<T> ArrayListConstIterator<T>::operator--(int)
{
    ArrayListConstIterator<T> temp(mPtr);
    --*this;
    return temp;
}
...
ArrayListConstIterator<T> ArrayListConstIterator<T>::operator+(int offset) const
{
    const T* temp(mPtr);
    temp = temp + offset;
    ArrayListConstIterator<T> tempIter(temp);
    return tempIter;
}
...
{
    ArrayListConstIterator<T> temp(iter.mPtr);
    return temp + offset;
}
